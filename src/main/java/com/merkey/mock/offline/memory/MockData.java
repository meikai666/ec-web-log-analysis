package com.merkey.mock.offline.memory;
...
public class MockData {
    ...
    private static void mockProductInfos(JavaSparkContext sc, SQLContext sqlContext) {
        ...
        //缓存一下
        sqlContext.cacheTable("product_info");
    }
    ...
    private static void mockUserAccessAction(JavaSparkContext sc, SQLContext sqlContext) throws IOException {
        ...
        Dataset<Row> dataset = sqlContext.createDataFrame(rowsRDD, schema);
        //注意：此时user_visit_action不是hive表，是内存中的虚拟表，当前的应用执行完毕后，就丢失了。
        //什么时候使用此种方式？为了提高编程的速度。部署阶段数据流转的过程可以使用其来代替。
    ...
    private static void randomAccessPage(List<Row> rows, String date, long userId, String sessionId) throws IOException {
        ...
        for (int k = 0; k < random.nextInt(100); k++) {
            ...
            if ("search".equals(action)) {
                ...
                orderProductIds = String.valueOf(randomProductId);
                orderCategoryIds = String.valueOf(clickProductIdMap.get(randomProductId));
