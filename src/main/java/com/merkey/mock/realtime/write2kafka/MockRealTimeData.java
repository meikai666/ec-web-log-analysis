package com.merkey.mock.realtime.write2kafka;
...
/**
 * Description：模拟实时产生的数据 （本地集群模式，将实时产生的数据直接发布到kafka集群中）<br/>
...
public class MockRealTimeData extends Thread {
    ...
    public MockRealTimeData() {
        ...
        try {
            InputStream stream = MockRealTimeData.class.getClassLoader().getResourceAsStream("producer.properties");
    ...
    public void run() {
        ...
        while (true) {
            ...
            producer.send(new ProducerRecord<Integer, String>("ad_log", log));
